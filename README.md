...
2. Figs_3ab_S4_S5 (including the information for Figs. S4 and S5 and Tables S1 and S2)
 a) Bacteria_NMDS
   ...
   NMDS_scores_bacteria.csv / data.scores_seagrassD4_Bacteria_cnd_env_new.csv
   (code for plot)
   NMDS_graph.py
 b) Eukaryotes_NMDS
   ...
   (code for plot)
   NMDS_graph.py
 c) Bacteria_alpha_diversity
   (raw_data stored on the related site)
   Bacteria_alpha_diversity_result_stat.csv  / List_feature_alpha_diversity_group_new.csv
   (code for plot)
   alpha_diversity_plot_cnd.py 
   stat_alpha_diversity.py
 d) Eukaryotes_alpha_diversity
   (raw_data stored on the related site)
   Eukaryota_alpha_diversity_result_stat.csv  / List_feature_alpha_diversity_group_new.csv
   (code for plot)
   alpha_diversity_plot_cnd.py 
   stat_alpha_diversity.py
...
5. Fig_5a
   ...
   (code)
   CLR_transform.py / Causal_learning_Fisherz_final.py
   
5. Fig_5b
   (raw_data stored on the related site)
   causal_learning_data_seagrass_CLR_nonEy_target
   (code)
   SR_target_only_maxdepth_loop / Model_lowest_table.py / analyze_hof_stability.py / Csv_bind_and_heatmap_colourfulnew.py
   
6. Fig_6_S18 (including the information for Fig.S18,and Tables S4 and S5)
   (raw_data stored on the related site)
   causal_learning_data_seagrass_CLR_region_site.csv 
   (code)
   SR_deterministic_target_only_maxdepthnew.py / SR_evaluation_whole_folder.py / SEM_Seagrass_CLR_final260126.py / Path260125.py
...
7. Supplementary information
   ...
   16S_D4_count.csv
   18S_D4_count.csv
   Bacteria_D4_raw_data.csv
   Bacteria_D5_raw_data.csv
   Eukaryota_D4_raw_data.csv
   Eukaryota_D5_raw_data
   Seagrass_list_cnd.csv
   significant_results_Bacteria_D4.tsv
   significant_results_Eukaryota_D4.tsv
   (code)
   masilin3_commands.R

   Fig_S14
   (raw_data for plot,stored on the related site)
   causal_learning_data_seagrass_CLR260115new_target.csv
   (code)
   PM-network_graph.py
   
   Fig_S15
   (raw_data for plot,stored on the related site)
   causal_learning_data_seagrass_CLR260115new.csv
   (code)
   Corelation_final260119.R
   
   Fig_S16 (including the information for Table S3)
   (raw_data for plot,stored on the related site)
   causal_learning_data_seagrass_CLR260115new.csv
   (code)
   EFA_seagrass.R
   ...
   Fig_S19 (including the information for Tables S6 and S7)
   (raw_data for plot,stored on the related site)
   causal_learning_data_seagrass_CLR_region_site.csv
   (code)
   Seagrass_SR_Desulf_seagrass260126.R / Seagrass_SR_seagrass_Desulf260126.R

   Fig_S20
   (raw_data for plot,stored on the related site)
   causal_learning_data_seagrass_CLR_region_site.csv
   (code)
   BayesLingam_seagrass_SR_260112.R 
